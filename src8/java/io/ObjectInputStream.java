 ...
 * ObjectInputStream 对先前使用 ObjectOutputStream 编写私有数据和对象进行序列化
 *
 ...
 * 与 FileInputStream 、 FileOutputStream 一起使用时，ObjectOutputStream 和
 * ObjectInputStream 可以为对象图提供持久存储的应用程序。ObjectInputStream 用于恢复以前序列化的对象
 *
 * <p>ObjectInputStream ensures that the types of all objects in the graph
 * created from the stream match the classes present in the Java Virtual
 * Machine.  Classes are loaded as required using the standard mechanisms.
 * ObjectInputStream 确保从流创建的图中的所有对象的类型与 Java虚拟机中存在的类相匹配。
 * 使用标准机制根据需要加载类。
 *
 ...
 * readObject 方法用来从流中读取对象。应使用Java 安全的类型转换来获取所需类型。
 * 在Java中，字符串和数组是对象，在序列化期间被视为对象。读取时，需要将它们转换为预期的类型。
 *
 ...
 * 对象的默认反序列化机制将每个字段的内容恢复为其写入时的值和类型。反序列化的过程将忽略声明
 * 为 transient 或 static 的字段。
 *
 *
