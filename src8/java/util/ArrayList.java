 ...
 * 实现了List接口的可扩容的数组(动态数组)，实现所有可选择的列表操作，允许所有的元素，包括空值
 * 除了使用List接口的功能外，ArrayList 还提供了内部存储list的方法。ArrayList能够完全替代
 * Vector，除了ArrayList 不是线程安全外。
 *
 ...
 * size(),isEmpty(),get(),set(),iterator() 和 listIterator() 操作运行恒定的时间
 * add()操作在分摊的常量时间中运行。
 * 这是因为，增加n个元素需要O(n)的时间，所有其他的操作运行线性的时间
 *
 ...
 * time cost.
 * 每一个ArrayList实例都有一个容量的概念，这个数组的容量就是list用来存储元素的。这个capacity
 * 总是能够快速增长，依照元素被添加进如ArrayList后，它总是自动地进行增长。
 *
 * <p>An application can increase the capacity of an <tt>ArrayList</tt> instance
 * before adding a large number of elements using the <tt>ensureCapacity</tt>
 * operation.  This may reduce the amount of incremental reallocation.
 * 程序能够增加ArrayList示例的容量在添加并使用 ensureCapacity 一大部分元素操作之前。
 * 这可能会减少增量重新分配的数量。
 *
 ...
 * 注意这个实现类不是线程安全的，如果多个线程中至少有两个线程修改了ArrayList的结构的话
 * 那么最终ArrayList中元素的个数和值可能就会发生变化。
 *
 ...
 * 如果不存在此类对象，则应使用Collections.synchronizedList"包装"该列表。
 * 最好在创建时期就这么做,防止意外的不同步列表访问。
 *
 * <p><a name="fail-fast">
 ...
 * time in the future.
 * iterator和listIterator 都会返回iterators，并且都是支持fail-fast机制的。
 * 如果创建后再进行结构化修改的话，除了通过迭代器自己以外的任何方式，都会抛出ConcurrentModificationException
 * 因此，面对并发修改的情况，iterator 失败的快速而且干净，而不是在未来的未确定时间冒任意，非确定性行为的风险。
 *
        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     * 空数组的实例用来存储默认大小的空实例。分辨与EMPTY_ELEMENTDATA 的区别是知道它的最大值
     * 当第一个元素被添加的时候
     *
     */
     ...
     * 数组缓冲区用来真正存储List数据的元素
     * ArrayList的初始容量是数组缓冲区的长度，任何空的ArrayList中的elementData ==
     * DEFAULTCAPACITY_EMPTY_ELEMENTDATA 会在DEFAULT_CAPACITY 的基础上进行扩容
     * 当第一个元素被添加的时候
     *
     */
     ...
     * iterator.
     * 构造函数，可以接受一个Collection(只要实现了Collection的类就可以)
     *
    ...
    private void ensureCapacityInternal(int minCapacity) {
        ...
        if (elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA) {
            // 则minCapacity 取默认初始容量和minCapacity 的最大值
            minCapacity = Math.max(DEFAULT_CAPACITY, minCapacity);
        }
        // 确保清晰的容量(最小容量与List元素的比较)，进行元素填充
        ensureExplicitCapacity(minCapacity);
    }
     ...
     * 检查给出的元素是否在范围内，如果没有，抛出适当的运行时异常。这个方法不会检查下标
     * 是负数。
     *
     */
