 ...
 *
 * 一个无界的基于链表的 TransferQueue。这个队列对任何给定的生产者进行 FIFO 排序
 * head 是队列中存在时间最长的元素。tail 是队列中存在时间最短的元素。
 *
 ...
 *
 * 意识到这一点，不像是大部分的集合一样，size 方法不是恒定的时间开销。由于这些队列的
 * 异步性质，确定当前元素需要对元素进行便利，因此，如果在遍历期间修改了此集合，则可能会报告不正确的结果
 *
