 ...
 * 此类只包括静态方法操作或者返回collections。它包含对集合进行操作的多态算法，“包装器”，
 * 它返回由指定集合支持的新集合，以及其他一些可能的结果。
 *
 * <p>The methods of this class all throw a <tt>NullPointerException</tt>
 * if the collections or class objects provided to them are null.
 * 此类的方法都会抛出空指针异常，如果提供给colletions或者类对象是null的话。
 *
 ...
 * 此类包含的多态算法的文档，通常包括实现的简要描述。这样的描述应视为实现说明，而不是特殊说明
 * 实现类可以自由替换其他算法，只要规范本身得到遵守。（例如，这个算法使用sort 而不是需要去使用归并排序
 * ，但是依旧很稳定）
 *
 ...
 * 此类也包括"破坏性"的算法，这是因为，这个算法修改了它们操纵的集合，抛出UnsupportedOperationException
 * 如果这个collection 不支持原始修改，就像set方法。这些算法可能，但不是必须的，抛出异常如果调用
 * 的方法没有对集合产生影响。例如，调用sort方法在不可修改的list，就可能会抛出UnsupportedOperation异常。
 *
...
public class Collections {
     ...
     * 对指定的列表进行升序排序，根据默认的(Comparable)排序规则，所有列表中的元素都必须实现
     * Comparable 接口，此外，列表中呢所有的元素必须互相排序，这也就是说，e1.compareTo(e2)
     * 必须不能抛出 ClassCastException 对于列表中任意元素来说
     *
     * <p>This sort is guaranteed to be <i>stable</i>:  equal elements will
     * not be reordered as a result of the sort.
     * 这个排序保证稳定性，相同的元素不会因排序而重新排序
     *
     ...
     * 用二分查找在指定列表中搜索指定对象。列表必须按升序排序根据其(Comparable 自然排序)之前
     * 的元素(如 list.sort() 方法)调用。如果未排序，则结果未定义。如果列表包含多个指定
     * 对象的元素，无法确保找到哪一个。
     *
