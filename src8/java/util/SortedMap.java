 ...
 * 一个提供了在key上全部排序的map。这个map根据key的Comparable 自然排序，或者通过Comparator
 * 提供了在创建时期的map排序。遍历有序映射的集合片段(由entrySet、keySet和values 方法返回)
 * 时能够反应此顺序的影响。
 *
 *
 ...
 * 所有插入到sorted map的key都必须实现Comparable 接口(或者接受定制的Comparator 接口)。
 * 进一步来讲，所有的key都必须相互比较形如 k1.compareTo(k2) 或者 comparator.compare(k1,k2)
 * 对其中的任何key都不能抛出ClassCastException。试图违反此限制将导致违规方法或构造函数抛出
 * ClassCastException。
 *
