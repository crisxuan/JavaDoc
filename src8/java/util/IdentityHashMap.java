 ...
 * 此类使用哈希表实现Map接口，在比较键和值时使用引用相等性替换对象相等性。换句话说，在IdentityHashMap
 * 两个key，k1和k2 被认为是相等的当且仅当 k1 == k2时。(在正常的Map实现中(类似HashMap)，两个键
 * k1 和 k2 被认为时相等的当且仅当 k1 == null ? k2 == nnull : k1.equals(k2)时)
 *
 ...
 * 这个类不是一个通用的Map实现！虽然这个类实现了Map接口，但它故意违反了Map的约定，
 * 该约定要求在比较对象时使用equals方法，此类仅适用于需要引用相等语义的极少数情况。
 *
 ...
 * 此类的典型用法是拓扑保留对象图转换，例如序列化或深度复制。要执行此类转换，程序必须维护一个“节点表”，
 * 以跟踪已处理的所有对象引用。节点表必须不等同于不同的对象，即使它们碰巧相等。 此类的另一个典型用法是维护代理对象。
 * 例如，调试工具可能希望为正在调试的程序中的每个对象维护一个代理对象。
 *
 * <p>This class provides all of the optional map operations, and permits
 * <tt>null</tt> values and the <tt>null</tt> key.  This class makes no
 * guarantees as to the order of the map; in particular, it does not guarantee
 * that the order will remain constant over time.
 * 这个类提供所有可选择的map操作，并且允许null值和null key。这个类不保证map的顺序，特别的，
 * 它不保证顺序会随着时间的推移保持不变。
 *
 ...
 * 这个类为基础的操作(get 和 put)提供了恒定的时间，假定系统标示函数 System.identityHashCode(Object)
 * 在桶之间正确地分散元素。
 *
