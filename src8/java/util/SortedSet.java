 ...
 * 一个进一步对元素进行排序的set。使用Comparable 对元素进行自然排序护或者使用Comparator在创建时对元素
 * 提供定制的排序规则。set的迭代器将按升序元素顺序遍历集合。
 *
 ...
 * 所有插入到SortedSet中的元素都必须实现Comparable接口(或者接口定制的构造器)。进一步来说，
 * 所有内部的元素都必须相互比较：e1.compareTo(e2) 或者comparator.compare(e1, e2)并且不会抛出
 * ClassCastException。尝试违反此限制将导致违规方法或构造函数抛出 ClassCastException。
 *
 ...
 * 请注意：如果排序集要正确实现Set 接口，则排序集维护的排序必须于equals一致。(查阅Comparable
 * 接口或者Comparator 接口)。这是因为Set接口是根据equal操作定义的。但是有序集合使用compareTo
 * 执行所有元素的比较。因此从排序集的角度看，这种方法认为相等的两个元素是相等的。
 * 即使排序与equals不一致，排序集的行为也是良好定义的。它只是不遵守一般的Set接口定义。
 *
 ...
 * 2) 一个创建了单个Comparator类型参数的构造函数，它创建一个根据指定比较器排序的空排序集
 * 3) 一个创建了单个Comparator类型参数的构造函数，它创建一个新的有序集合，其元素与其参数相同，
 * 并根据元素的自然顺序进行排序
 * 4）一个创建了单个SortedSet类型参数的构造器，它创建了一个新的有序集，其元素和输入有序集的顺序相同。
 * 由于接口不能包含构造函数，因此无法强制执行此建议
 *
