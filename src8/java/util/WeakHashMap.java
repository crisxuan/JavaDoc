 ...
 * implementations.
 * 基于哈希表的Map基础实现，带有弱键
 * WeakHashMap 中的entry 当不再使用时还会自动移除。更准确的说，给定key的映射的存在将不会阻止
 * key被垃圾收集器丢弃，即，可以最终化，finalized然后回收。
 *
 * <p> Both null values and the null key are supported. This class has
 * performance characteristics similar to those of the <tt>HashMap</tt>
 * class, and has the same efficiency parameters of <em>initial capacity</em>
 * and <em>load factor</em>.
 * 既支持空值也支持空元素。该类具有与HashMap类相似的性能特征，并具有初始容量和加载因子相同
 * 的效率参数。
 *
 ...
 * method.
 * 像大部分collection类，这个类也不是线程安全的。
 * 一个线程安全的WeakHashMap使用Collections.synchronizedMap 方法构造。
 *
 ...
 * 此类主要用于使用 == 运算符 equals 方法测试对象标示的关键对象。一旦这样的key被丢弃，
 * 它就永远不会被重新创建，因此以后不可能在WeakHashMap中查找到该key，并对其被移除感到不可思议
 *
